...
class tracking_pub(Node):
    def __init__(self):
        ...
        self.cross_4pts = np.loadtxt(cur_path+'/crosswalk/crosswalk_with_id_2.txt',dtype=np.float32,delimiter=',')
    ...
    def get_crss_box(self, obj, ego): 
        ...
        print('in',poly.contains(Point([x,y])))
        if poly.contains(Point([x,y])):
            local_ctr = (crss_loc[0]+crss_loc[3])/2  #후륜중심 기준
            # return wireframe(crss_loc[0],crss_loc[1],crss_loc[2],crss_loc[3],z=1), local_ctr
            ...
            box_heading = np.arctan2(heading_vector[1],heading_vector[0]) - head
            return [local_ctr[0]-1.0, local_ctr[1], 0.6, length, width, 2.0, box_heading],None  #라이다 좌표 기준
        else:
            return None,None
        ...
        if cur_object.shape[0] != 0:
            ...
            for i, cur_ in enumerate(cur_object):
                ...
                if self.ego_info is not None:
                    self.crss_box, box_ctr = self.get_crss_box([cur_[1],cur_[2]],self.ego_info)
                # print(cur_, self.crss_box)
                if cur_[-1] == 2:  #person
                    head = self.ego_info[2]
                    x, y = cur_[1]+1,cur_[2]
                    R = np.array([[cos(head), -sin(head), 0],[sin(head), cos(head), 0], [0, 0, 1]])
                    obj_global_loc = np.round((np.matmul(R, np.array([x,y,0]).T).T)[:2]+self.ego_info[:2],2)
                ...
                if self.crss_box is not None: # crsswalk
                    ...
                    # tracking_boxes.points = tracking_boxes.points + self.crss_box
                    tracking_boxes.color.r, tracking_boxes.color.g, tracking_boxes.color.b = rainbow_colormap[6]
                    info = Object()
                    # info.point.x, info.point.y, info.point.z = box_ctr[0],box_ctr[1],0.6
                    info.point.x, info.point.y, info.point.z = self.crss_box[0:3]
                    info.point.x += 1.0  #lidar 후륜중심 offset
                    info.l, info.w, info.h = self.crss_box[3:6]
                    info.yaw = self.crss_box[6]
